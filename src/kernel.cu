...
int main(int argc, char** argv) {
    ...
    std::ofstream file;
    file.open("logs/ising.log");
    if (file.is_open()) file << "# iteration | market value" << std::endl;
    ...
    for (int iteration = 0; iteration < total_updates; iteration++) {
        ...
        if (iteration % 1000 == 0) {
            cudaMemcpy(h_global_market, d_global_market, sizeof(*d_global_market), cudaMemcpyDeviceToHost);

            if (file.is_open()) {
                file << iteration << ' ' << h_global_market[0] << std::endl;
            }
        }
    }
